...
type HomeTab = 'tournament' | 'league' | 'open'
...
export default function HomePage() {
  const [activeTab, setActiveTab] = useState<HomeTab>('tournament')
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="flex-1 flex items-center justify-center p-5 relative z-10">
        <div className="w-full max-w-sm stagger">
          ...
          {/* Worlds 2026 */}
          <div className="relative rounded-2xl overflow-hidden mb-5 neon-border border border-brand/10 bg-surface-3/80">
            <div className="absolute inset-0 bg-gradient-to-br from-brand/5 via-transparent to-transparent pointer-events-none" />
            <div className="relative px-5 py-3.5 flex items-center gap-4">
              <div className="home-worlds-badge h-10 shrink-0 flex items-center justify-center">
                <img
                  src="/img/worlds-2026.png"
                  alt="Roundnet World Championship 2026"
                  className="h-9 home-worlds-logo"
                />
              </div>
              ...
              {activeTab === 'tournament' && (
                <form onSubmit={handleCreate} className="space-y-3" key="tournament">
                  <input
                    ...
                    className="w-full px-4 py-3.5 bg-surface-3 border border-border rounded-xl text-white placeholder-zinc-600 focus:outline-none focus:border-brand/50 focus:shadow-[0_0_20px_rgba(228,255,26,0.08)] transition-all text-base"
                    maxLength={200}
                    autoFocus
                  />
              ...
              {activeTab === 'open' && (
                <form onSubmit={handleOpen} className="space-y-3" key="open">
                  <input
                    ...
                    className="w-full px-4 py-3.5 bg-surface-3 border border-border rounded-xl text-white placeholder-zinc-600 focus:outline-none focus:border-brand/50 transition-all text-base"
                    autoFocus
                  />
                  <button
                    type="submit"
                    disabled={!slug.trim()}
                    className="w-full px-6 py-3.5 border border-border-bright text-white font-display font-bold uppercase tracking-wider rounded-xl hover:bg-surface-3 hover:border-brand/30 disabled:opacity-30 disabled:cursor-not-allowed transition-all text-sm cursor-pointer"
                  >
                    Open
                  </button>
                </form>
      ...
      <footer className="py-4 flex items-center justify-center">
        <span className="text-[10px] font-display uppercase tracking-[0.3em] text-zinc-600">
